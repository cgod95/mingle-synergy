...
const item = (isActive:boolean) =>
  `flex-1 py-2 text-center text-sm min-h-[44px] flex items-center justify-center ${isActive ? "font-semibold text-indigo-400" : "text-neutral-500"}`;

export default function BottomTab() {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-20 border-t border-neutral-700/50 nav-blur-ios md:hidden hide-on-keyboard" style={{ paddingBottom: 'max(8px, env(safe-area-inset-bottom, 0px))' }}>
